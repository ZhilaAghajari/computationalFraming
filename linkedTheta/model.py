...
class ThetaRoleModel:
    def __init__(self, corpus, originaltext, doc_relns, doc_arg2, vocab_relns, vocab_args, id2word, reln2id, arg2id, idx2docid, n_iters, K, T, D, V, R, A2, alpha, eta, etaprime, gamma, lam, omega, corpusName):
        self.corpus = corpus
    ...
    def initialize_variables(self):
        ...
        self.n_treln_k = np.zeros((self.T, self.K))
        self.n_targ_k = np.zeros((self.T, self.K))
        ...
        for d, doc in enumerate(self.corpus):
            ...
            for i, word_id in enumerate(doc):
                ...
                self.n_treln_k[y1, z]+=1
                self.n_targ_k[y2, z]+=1
...
#                   
                    ...
                    p_z_t = (self.n_t_k[:, z]) \
                                 / (np.sum(self.n_t_k[:,z]))
                    p_z_trel = self.n_treln_k[:, z] \
                                /(np.sum(self.n_treln_k[:, z]))
                    ...
                    #p_t_reln_k = np.sum(p_t_reln, axis =1)*p_z_t 
                    p_t_reln_k = np.sum(p_t_reln, axis =1)*p_z_trel 
                     
                    
                    p_z_targ = self.n_targ_k[:, z] \
                                /(np.sum(self.n_targ_k[:,z]))
                    for arg in arg2s:
                    ...
                    p_t_arg2_k = np.sum(p_t_arg2, axis = 1)*p_z_targ
                    ...
                    for reln in relns:
                        ...
                        self.n_w_reln[idx, self.reln2id[reln]]+=1
                    for arg in arg2s:
                        #self.n_t_arg2[y1, self.arg2id[arg]] += 1
                        self.n_t_arg2[y2, self.arg2id[arg]] += 1
                        self.n_w_arg2[idx, self.arg2id[arg]]+=1
#                         self.n_w_arg[idx,arg]+=1
    ...
    def compute_phi(self):
        '''
        Compute the phi matrix from the plate diagram ... OLD ONE
        '''
    ...
    def compute_phi_reln(self):
        '''
        Compute the phi matrix from the plate diagram for reln component  
        '''
        self.phi_reln = np.zeros((self.K, self.T))
        for k in range(self.K):
            for t in range(self.T):
                self.phi_reln[k, t] = (self.n_treln_k[t, k] + self.gamma) / (np.sum(self.n_treln_k[:, k]) + self.T * self.gamma)  
        return self.phi_reln
    
    def compute_phi_arg(self):
        '''
        Compute the phi matrix from the plate diagram for arg2 component
        '''
        self.phi_arg = np.zeros((self.K, self.T))
        for k in range(self.K):
            for t in range(self.T):
                self.phi_arg[k, t] = (self.n_targ_k[t, k] + self.gamma) / (np.sum(self.n_targ_k[:, k]) + self.T * self.gamma)  
        return self.phi_arg
    ...
    def print_all_results(self):
        ...
        '''
        df = pd.DataFrame(columns = ['topic_id', 'word', 'relns', 'args'])
        ls = []
        top_doc = 10
        tops = 1
        for k in range(self.K):
            ...
            top_theta_reln_ids = self.top_theta_role_reln_for_topic(k, tops)
            #maybe we should get 3 top theta role instead of one top theta ?
            print(' *** Top theta roles {0} - {1} , for reln component ** \n'.format(str(top_theta_reln_ids[0]), str(self.phi_reln[k, top_theta_reln_ids[0]])))
            print('Distribution of reln for each words of the topic {0}'.format(k))
            for t in top_theta_reln_ids:
            ...
            top_theta_arg_ids = self.top_theta_role_arg_for_topic(k, tops)
            print(' *** Top theta roles {0} - {1} , for arg component ** \n'.format(str(top_theta_arg_ids[0]), str(self.phi_arg[k, top_theta_arg_ids[0]])))
            for t in top_theta_arg_ids:
            ...
            print('Distribution of arg2 and relns for each words of the topic {0}'.format(k))
            print('-------- -------- -------- -------- -------- --------')
            for widx in  word_ids:
                ...
                results_arg = sorted(dic_arg2.items(), key=lambda x: x[1], reverse=True)
                print(f"* {str(self.id2word[widx])}: Arg2: {', '.join([result[0]+':'+str(result[1]) for result in results_arg[0:20]])} \n")
                ...
                print(f"* {str(self.id2word[widx])}: Relns: {', '.join([result[0]+':'+str(result[1]) for result in results_relns[0:20]])} \n")
                print('-------- -------- -------- -------- -------- --------')
                
                #@@@@@@@@
                #Store components for this topic terms into ls
                ls.append({'topic_id':k,'word':self.id2word[widx],\
                          'relns':','.join([result[0] for result in results_relns[0:20]]),\
                          'arg2':','.join([result[0] for result in results_arg[0:20]])})
#                 ls.append({'topic_id':k,'word':self.id2word[widx],\
#                           'relns':','.join([result[0]+':'+str(result[1]) for result in results_relns[0:10]]),\
#                           'arg2':','.join([result[0]+':'+str(result[1]) for result in results_arg[0:10]])})
        #storeTopics()
        df = pd.DataFrame(ls)
        #df.to_csv('topicsResults_sample_5.csv')
        df.to_csv('topicsResults_{0}_topic_{1}_thetarole_{2}.csv'.format(self.K,self.T,self.corpusName))
        del ls
    ...
    def arg2_distribution_for_word_idx(self, widx, t):
        ...
        for arg in self.vocab_args:
            ...
            p_w_arg= self.n_w_arg2[widx, self.arg2id[arg]]/np.sum(self.n_w_arg2[:, self.arg2id[arg]])
    ...
    def reln_distribution_for_word_idx(self, widx, t):
        '''
        calculate p(reln|word, theta, topic) but givn it is calculated for topic terms, we remove topic here..
        '''
        res ={}
        for reln in self.vocab_relns:
            p_w_reln = self.n_w_reln[widx, self.reln2id[reln]]/np.sum(self.n_w_reln[:, self.reln2id[reln]])
    ...
    def top_theta_role_reln_for_topic(self, k, tops):
        top_theta_indx = np.argsort(self.phi_reln[k, :])[::-1][:tops]
        return top_theta_indx
    
    def top_theta_role_arg_for_topic(self, k, tops):
        top_theta_indx = np.argsort(self.phi_arg[k, :])[::-1][:tops]
        return top_theta_indx
...
# add the top documents as well
    ...
    def print_top_documents_topic(self, top_doc=10):
        ...
        ls = []
        for k in range(self.K):
            top_documents_indx = np.argsort(self.theta[:, k])[::-1][:]
            #top_documents_indx = np.argsort(self.theta[:, k])[::-1][:top_doc]
            top_thetas = 10
            print('topic {0} \n'.format(k))
            try:
                ...
                for idx in top_documents_indx:
                    if c<=top_doc:
                        print("doc {0} - {1} - {2} \n ".format(self.idx2docid[idx], self.theta[idx, k], self.originaltext[self.idx2docid[idx]]))
                    ls.append({'topic_id':k,'document_ids':self.idx2docid[idx],\
                          'probability_score':self.theta[idx, k]})
                    c+=1
                print('------------------------------------------------------------------------')
            except:
                print('An error occured here in TOP DOCUMENTS.......')
#                 return top_documents_indx
#                 return np.argsort(self.theta[:, k])[::-1]
        #store probability of all documents for each topic in a dataframe and in csv file.
        df = pd.DataFrame(ls)
        #df.to_csv('topicsDocProbabilities.csv')
        df.to_csv('topicDocProbabilities_{0}_topic_{1}_thetarole_{2}.csv'.format(self.K,self.T,self.corpusName))
        del ls
    ...
    def compute_matrices(self):
        ...
        _phireln = self.compute_phi_reln()
        _phiarg = self.compute_phi_arg()
    ...
    def print_all(self):
        print('TOPICS TOP TERMS, TOP GRAMMATICAL RELN, and TOP ARG2 distributions ****************** \n ')
